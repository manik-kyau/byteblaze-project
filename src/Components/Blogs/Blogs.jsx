	...
	<div className="container max-w-6xl p-6 mx-auto space-y-6 sm:space-y-12">
		<a rel="noopener noreferrer" href="#" className="block max-w-sm gap-3 mx-auto sm:max-w-full group hover:no-underline focus:no-underline lg:grid lg:grid-cols-12 dark:bg-gray-900">
			...
			<div className="p-6 space-y-2 lg:col-span-5">
				...
				<span className="text-xs dark:text-gray-400">{blogs[0].description}</span>
				<p>{new Date(blogs[0].published_at).toLocaleDateString()}</p>
			</div>
		</a>
		<div className="grid justify-center grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{
                blogs.slice(1).map((blog,idx) =><Blog key = {idx} blog={blog}></Blog>)
            }
		</div>
	</div>
</section>
